...
public:
  ...
  int elen = 0; // efective length of the transcript
  void store_elen(){ // computes effective length of the transcript
      this->elen = 0;
      for(int i=0;i<this->exons.Count();i++){
          this->elen+=this->exons.Get(i)->len();
      }
  }

  float num_reads = 0; // number of reads that were assigned to this transcript
