...
bool passes_pair_flags(GSamRecord* brec){
    ...
    if(options.flags==0){ // no extra flags specified - can proceed
        return true;
    }
    //  need to check user options
    if(options.flags & 0x2){ // user requested valid pairs only
        if(!(brec->get_b()->core.flag & 0x2)){ // current read is not mapped as a valid pair
            return false;
        }
    }
    if(options.flags & 0x4 || options.flags & 0x8){ // user requested these flags to be required for pairing
...
int main(int argc, char *argv[])  {
    ...
    while ((irec=inRecords.next())!=NULL) {
        ...
        if(options.index){
            ...
            if(std::strcmp(bam_get_qname(brec->get_b()),"SRR1071717.27")==0){
